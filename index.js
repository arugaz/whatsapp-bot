...
<<<<<<< HEAD
    ...
    if (event.action === 'add' && event.who !== host) {
        ...
        const capt = `*ey yo,what up!* *@${event.who.replace('@c.us','')}*\n\n*Welcome to Our ${name}\nHave Fun In ${name} Group.\n\nCommands bot #help , #menu`
    }
    // kondisi ketika seseorang dikick/keluar dari group
    if (event.action === 'remove' && event.who !== host) {
        const burik = await aruga.getProfilePicFromServer(event.who)
        const hasu = `busettt, eh @${event.who.replace('@c.us', '')} udah dipungut malah mau jadi anak pungut lagi.`
...
>>>>>>> b13bae9d5f383177ca046886717ccd82b206356d
    ...
    aruga.onIncomingCall(async (callData) => {
        ...
        await aruga.sendText(callData.peerJid, 'Sorry, who call or vc Bot will got automate block\n\n-bot')
        .then(async () => {
