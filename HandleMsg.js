...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        let { body } = message
        var { name, formattedTitle, gcnname, namagc } = chat
...
${desc}`)
            ...
            const zodiak = await rugaapi.zodiak(args[0],args[1],args[2])
            await aruga.reply(from, zodiak, id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case 'resep':
            ...
            case 'nekopoi':
                if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                if (args.length === 1) return aruga.reply(from, 'Kirim perintah *#nekopoi [linkNekopoi]*\nContoh : *#nekopoi https://nekopoi.care/tsunpuri-episode-1-subtitle-indonesia/*', id)
                try {
	...
	case 'lirik':
        ...
        if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
        if ((!isOwner) && (isOwnerB)) return aruga.reply(from, 'Maaf, perintah ini hanya bisa dilakukan oleh Owner Grup')
        let isOwnerBot = chat.groupMetadata.owner == pengirim
