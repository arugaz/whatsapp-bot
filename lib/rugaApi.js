...
const cuaca = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://rest.farzain.com/api/cuaca.php?id=j${url}&apikey=O8mUD3YrHIy9KM1fMRjamw8eg`)
    .then((res) => {
		if (res.data.result.cuaca == null) resolve('Maaf daerah kamu tidak tersedia')
...
const tulis = async (teks) => new Promise((resolve, reject) => {
    axios.get(`${link}/nulis?text=${encodeURIComponent(url)}`)
    .then((res) => {
        resolve(`${res.data.result}`)
    })
    .catch((err) => {
