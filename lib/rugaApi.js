...
const kodepos = async (kelurahan,kecamatan) => new Promise((resolve, reject) => {
	axios.get(`https://kodepos.herokuapp.com/?q=${kelurahan}+${kecamatan}`)
	.then((res) => {
		if (res.data.success == false) resolve('Maaf daerah kamu tidak tersedia')
		const tekspos = `Provinsi: ${res.data.data[0].province}\nKota/Kabupaten: ${res.data.data[0].city}\nKecamatan: ${res.data.data[0].subdistrict}\nKelurahan: ${res.data.data[0].urban}\nKode Pos: ${res.data.data[0].postalcode}`
		resolve(tekspos)
	})
	.catch((err) => {
		reject(err)
