...
export default <Command>{
  ...
  execute: async ({ aruga, message, prefix, args, user }) => {
    const listLanguages: {
      ...
      await readFile(pathJoin(__dirname, "..", "..", "..", "database", "languages.json"), "utf-8"),
    );
    ...
    if (args.length >= 1 && !!listLanguages.find((value) => value.iso === args[0])) {
      ...
      return await message.reply(
        `${i18n.translate(
          "commands.general.language.changed",
          { "@LANGUAGE": lang.lang },
          user.language,
        )}`,
        true,
    ...
    const text =
      ...
      "┏━━「 𓆩 𝐻ɪᴅᴅᴇɴ 𝐹ɪɴᴅᴇʀ ⁣𓆪 」\n" +
      "┃\n" +
      `┃ ${i18n.translate("commands.general.language.text.one", {}, user.language)}\n` +
      `┃ ${i18n.translate("commands.general.language.text.two", {}, user.language)}\n` +
      `┃ ${i18n.translate("commands.general.language.text.three", {}, user.language)}\n` +
      "┃\n" +
      `┗━━「 ꗥ${config.name}ꗥ 」` +
      "\n\n";
